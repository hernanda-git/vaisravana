"""Project Vaiśravaṇa — symbol registry, 1000x mapping, liquidity filter (doc 30 §2, doc 32).

Pure-logic component: no network required. The live Binance client is injected via the
`ExchangeClient` protocol so everything here is testable offline (paper/shadow default).
"""
