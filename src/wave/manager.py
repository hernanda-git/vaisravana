...
from wave.models import Wave, WaveState, BiasReading, Candidate, Tick, TickContext
from wave.smczones import SMCZoneCache
from wave.bias import size_from_confidence
from wave.db import log_wave_open, log_wave_close, append_telemetry
from wave.structure import detect_structure
...
class WaveManager:
    ...
    def on_tick(self, wave: Wave, tick: Tick, ctx: TickContext,
        ...
        # Trail SL — lock in profit as the wave forms (expert surfing):
        #   peak_r >= 0.3  -> SL to entry (breakeven, never lose)
        #   peak_r >= 0.6  -> SL to +0.3R (bank partial profit)
        #   peak_r >= 1.0  -> SL to +0.6R (ride the bigger wave)
        ...
        if wave.peak_r >= 1.0:
            ...
            new_sl = entry + 0.3 * risk if wave.side == "BUY" else entry - 0.3 * risk
        ...
        # Breakeven floor (legacy, kept for safety)
        if wave.peak_r >= BREAKEVEN_FLOOR_R and wave.side == "BUY" and wave.sl_price < wave.entry_price:
            wave.sl_price = wave.entry_price
        if wave.peak_r >= BREAKEVEN_FLOOR_R and wave.side == "SELL" and wave.sl_price > wave.entry_price:
            wave.sl_price = wave.entry_price
